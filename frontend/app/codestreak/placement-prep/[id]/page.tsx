      ...
      try {
        ...
        if (activeT) {
          ...
          const completed = submissions.some(
            (sub: any) => sub.problemId === problemId?.toString(),
          );
            ...
            {activeTopic.questions && activeTopic.questions.length > 0 ? (
                  ...
                  >
                    ...
                    >
                      <div className="flex gap-4">
                        <span className="text-xl font-black text-primary/40 mt-0.5">
                          Q{index + 1}
                        </span>
                      ...
                      <div
                        className={`mt-1 p-1 rounded-full transition-colors ${expandedIndex === index ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                      >
                        {expandedIndex === index ? (
                          <ChevronUp size={20} />
                        ) : (
                          ...
                          <div className="bg-muted/40 p-6 rounded-2xl border border-border/50 relative">
                            ...
                            {typeof item.a === "string" ? (
                              ...
                              <div className="text-muted-foreground leading-relaxed text-lg space-y-6">
                                {(item.a as any).props && (
                                  <div>
                                    <strong className="text-foreground text-xl block mb-3 border-b pb-2">
                                      Props
                                    </strong>
                                    <ul className="list-disc pl-5 mt-2 space-y-2">
                                      {(item.a as any).props.map(
                                        (p: string, i: number) => (
                                          <li key={i}>{p}</li>
                                        ),
                                      )}
              ...
              <div className="py-20 text-center bg-card border border-border rounded-2xl flex flex-col items-center justify-center">
                ...
                <p className="text-muted-foreground text-lg">
                  We are working hard to add question stay tuned.
                </p>
              </div>
            )}
          </div>
