  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error("Failed to load machine coding questions", err);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#0a0a0a] transition-colors duration-300">
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-10 py-16 md:py-20">
        <PageHeader
          eyebrow="Machine Coding"
          title="Components I've"
          titleAccent="built by hand"
          subtitle="Frontend machine-coding challenges I've implemented — from requirements and component structure to the edge cases and concepts they test."
        />
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {filtered.map((item, i) => (
              ...
              >
                <Link
                  href={`${basePath}/machine-coding/${toSlug(item.title)}`}
                  ...
                  <div className="mt-auto pt-5">
                    ...
                    <div className="flex items-center justify-between mt-4 text-xs">
                      <span className="font-mono text-gray-500 dark:text-gray-400">
                        {item.estimatedTime || "Machine coding"}
                      </span>
        ...
        {/* Solution references */}
        {items.some((i) => i.githubSolution) && (
          <p className="mt-8 text-xs font-light text-gray-400 dark:text-gray-500">
            Solutions live in the{" "}
            <a
              href="https://github.com/shreyam91/Machine-coding"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 font-mono text-[#3b82f6] hover:underline"
            >
              Machine-coding repo <ExternalLink className="w-3 h-3" />
            </a>
          </p>
        )}
      </div>
