...
export default function VideosPage() {
      ...
      try {
        ...
        const normalized = data.map((v: any) => {
          ...
          if (!ytId && v.youtubeLink) {
            const match = v.youtubeLink.match(/(?:v=|\/embed\/|\.be\/)([a-zA-Z0-9_-]{11})/);
  ...
  const filteredVideos = videosData.filter(v => activeCategory === "All" || v.category === activeCategory);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <div key={video._id || video.id} className="group cursor-pointer flex flex-col h-full">
                      <div className="relative rounded-xl overflow-hidden mb-3 aspect-video border dark:border-white/10 border-black/10 shadow-sm bg-black group cursor-pointer" onClick={() => !playingVideoId && setPlayingVideoId(video._id || video.id)}>
                        {playingVideoId === (video._id || video.id) ? (
                          ...
                          <>
                            <Image src={video.thumbnail} alt={video.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 33vw, 25vw" className="object-cover group-hover:scale-105 transition-transform duration-500 opacity-90" />
                  ...
                  {filteredVideos.length === 0 && (
                    <div className="col-span-full py-16 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-muted-foreground mt-1">Try adjusting your category filter.</p>
                    </div>
                  )}
                </div>
              )}
