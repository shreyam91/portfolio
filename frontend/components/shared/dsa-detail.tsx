        ...
        <div className="w-full max-w-4xl mx-auto flex flex-col">
          <div className=" lg:p-2">
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                <div className="bg-muted/30 border border-border p-3 rounded-lg text-center">
                  ...
                  <p className="text-lg font-bold">
                    {activeQuestion.interviewEstimate.expectedTimeMinutes}m
                  </p>
                </div>
                <div className="bg-muted/30 border border-border p-3 rounded-lg text-center">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">
                  ...
                  <p className="text-lg font-bold">
                    {activeQuestion.interviewEstimate.thinkingTimeMinutes}m
                  </p>
                </div>
                <div className="bg-muted/30 border border-border p-3 rounded-lg text-center">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">
                  ...
                  <p className="text-lg font-bold">
                    {activeQuestion.interviewEstimate.codingTimeMinutes}m
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="prose dark:prose-invert max-w-none text-sm leading-relaxed mb-8 mt-6">
              <div className="whitespace-pre-line text-foreground/90 text-base">
                ...
                <ul className="list-none space-y-2">
                  {activeQuestion.constraints?.map((c: string, i: number) => (
                    <li key={i} className="flex items-center gap-2">
                      ...
                      <code className="bg-muted px-2 py-0.5 rounded-md text-foreground/80">
                        {c}
                      </code>
                    </li>
                  ))}
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-muted-foreground flex items-center gap-1.5">
                            <Clock size={14} className="text-green-500" /> Time
                          </span>
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                {activeQuestion.companies?.map((company: string, i: number) => (
                  <span
                    key={i}
                    className="px-3 py-1 bg-muted text-xs font-medium rounded-full text-foreground/70 border border-border/50"
                  >
                    {company}
                  </span>
                ))}
                    ...
                    <ul className="space-y-3">
                      {activeQuestion.edgeCases.map((edge: any, i: number) => (
                        <li key={i} className="text-sm">
                          <span className="font-mono text-xs bg-orange-500/10 text-orange-600 px-1.5 py-0.5 rounded">
                            {edge.case}
                          </span>
            ...
            <div className="flex flex-col border border-border rounded-xl shadow-sm overflow-hidden h-[400px]">
              <div className="py-2.5 px-4 flex flex-row items-center justify-between bg-[#252526] border-b border-[#1e1e1e]">
                <div className="flex items-center gap-3">
                  ...
                  <Badge
                    variant="outline"
                    className="font-mono text-[10px] uppercase tracking-wider bg-zinc-900/50 text-zinc-400 border-zinc-800 ml-1 sm:ml-2"
                  >
                    Java
                  </Badge>
                  ...
                  <div className="flex bg-zinc-900 rounded-md p-0.5 ml-2 overflow-x-auto hide-scrollbar">
                    {activeQuestion.approaches?.map(
                      (
                        approach: { name: string; code: string },
                        idx: number,
                      ) => (
                        <button
                          key={idx}
